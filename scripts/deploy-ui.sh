#!/usr/bin/env bash
# Build the video plugin UI and rsync it to .57.
#
# Pairs with the nginx vhost change that points `video.4950.store` at
# `/Users/local/var/polar-ui/video/`. Cross-page sidebar links (e.g.
# /dashboard.html) fall back to dock's /ui/dist via nginx try_files, so
# this dir only needs to carry video's own files.
#
# Required env:
...
#   DEPLOY_PATH   default: /Users/local/var/polar-ui/video
#
# Usage from the polar-video repo root:
...
DEPLOY_PATH="${DEPLOY_PATH:-/Users/local/var/polar-ui/video}"
