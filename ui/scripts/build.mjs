// Build the video plugin UI.
//
//   src/video-studio.ts  -> dist/scripts/video-studio.js   (esbuild bundle, ESM)
...
await esbuild.build({
    entryPoints: [path.join(srcDir, "video-studio.ts")],
    bundle: true,
    format: "esm",
    target: ["es2022"],
    platform: "browser",
...
// Copy public/ assets verbatim — video-studio.html and anything else the
...
console.log("built video plugin UI -> dist/");
